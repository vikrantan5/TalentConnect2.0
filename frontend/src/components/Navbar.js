...
import {
  ...
  ChevronDown, User, Settings, LogOut, GraduationCap, Shield,
} from 'lucide-react';
...
const Navbar = () => {
  ...
  // Admin sees only platform name + notifications + profile. No nav links.
  const NAV = isAdmin ? [] : USER_NAV;
    ...
    >
      ...
      <div className="mx-auto max-w-[1480px] px-4 lg:px-8">
        <div className="flex h-16 items-center justify-between gap-4">
          {/* Logo / Platform name */}
          <Link to={homePath} className="flex items-center gap-2.5 group" data-testid="nav-logo">
            ...
            <span className="hidden sm:flex flex-col leading-none">
              <span className="font-display text-[22px] text-ink-950 dark:text-white">
                Talent<span className="italic text-gradient-cyan">Connect</span>
              </span>
              <span className="text-[10px] uppercase tracking-[.22em] text-ink-500 dark:text-ink-300">
                {isAdmin ? 'admin · command center' : 'skill · mentor · grow'}
              </span>
            </span>
          </Link>

          {/* Desktop nav (hidden for admin) */}
          {!isAdmin && NAV.length > 0 && (
          ...
          <div className="flex items-center gap-2">
                ...
                {openProfile && (
                  ...
                  >
                    ...
                    <div className="py-1">
                      {!isAdmin && (
                        <>
                          <Link to="/profile" onClick={() => setOpenProfile(false)} className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm hover:bg-ink-100 dark:hover:bg-white/5" data-testid="profile-menu-profile">
                      ...
                      {isAdmin && (
                        <Link to="/admin" onClick={() => setOpenProfile(false)} className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm hover:bg-ink-100 dark:hover:bg-white/5" data-testid="profile-menu-admin">
            ...
            {/* Mobile toggle (only when there is nav) */}
            {!isAdmin && NAV.length > 0 && (
